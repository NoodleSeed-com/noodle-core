...
export const assistantAppearanceOverrideSchema: z.ZodType<AssistantAppearanceOverride> = z
  .object({ branding: brandingSchema.optional(), assistant: assistantUiSchema.optional() })
  .strict()
  .refine(
    hasLeafValue,
    'appearance override must contain at least one setting',
  ) as z.ZodType<AssistantAppearanceOverride>;
...
export function resolveAssistantAppearanceConfiguration(
  developer: AssistantAppearanceOverride | undefined,
  operator: AssistantAppearanceOverride | undefined,
): ResolvedAssistantAppearanceConfiguration {
  if (developer === undefined && operator === undefined) {
    return { effective: undefined, fallback: 'halo', provenance: {} };
  }
  const effective = mergeObjects(developer ?? {}, operator ?? {}) as AssistantAppearanceOverride;
  const provenance: Record<string, 'developer' | 'operator'> = {};
...
function collectLeafPaths(
  ...
  output: Record<string, 'developer' | 'operator'>,
): void {
...
function hasLeafValue(value: Readonly<Record<string, unknown>>): boolean {
  return Object.values(value).some((child) =>
    isPlainObject(child) ? hasLeafValue(child) : child !== undefined,
  );
}
