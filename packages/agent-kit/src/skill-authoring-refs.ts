...
export function renderAuthoringWorkflowReference(): string {
  return [
    ...
    '',
    'This declaration compiles schema metadata only. It does not create a submit tool, grant access, select retention or residency, provision storage, or make records available to MCP or the embedded assistant. Operators configure permitted lifecycle and access; changing the system of record is not a provider toggle. Keep source-owned operations on their existing connector tools. A schema declaration does not turn an arbitrary connector into a writable collection or promise synchronized/searchable records.',
    '',
    '## Boundaries',
    '',
