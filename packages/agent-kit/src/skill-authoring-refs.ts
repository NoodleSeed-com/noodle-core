...
export function renderAuthoringWorkflowReference(): string {
  return [
    ...
    '',
    'Before authoring an HTTP connector for public-web search, page extraction, places/local search, routing, weather, translation, or media discovery, inspect the generated SDK surface reference. If the matching Noodle capability is actually exported, use its one documented declaration and generated tools; do not invent a parallel provider connector or an assistant-only fetcher. Business tools may compose its documented operation. The same declaration serves both the embedded assistant and external MCP agents under the same authorization and policy.',
    '',
    'Explicit page reading is `webExtract("public_pages", { title: "Read public pages", description: "Read selected public pages as untrusted evidence.", provider: noodleManaged(), policy: { maxUrls: 3 } })`. Register it in `server(..., { capabilities: [pages], ... }, tools)`; the generated ordinary tool is `extract_public_pages` with `{ urls: [...] }`. Include the declaration in an assistant surface allowlist to expose it there. Composed business tools may use `ctx.capabilities.public_pages.extract({ urls: ctx.input.urls })` and return a normal object mapping. No model or business brief is hidden inside extraction.',
    'This extraction-only learning cohort is managed-only and off until the operator enables the exact app/environment with `noodle capabilities configure`. Use `inspect` for effective limits and `test --mode fixture|live` for explicit diagnostics. It reads bounded public HTTPS HTML, respects robots, and never browses authenticated pages, runs JavaScript, crawls recursively or adds visitor pages to knowledge. General search and BYO capability binding are not shipped by this cohort. Keep provider names, endpoints and credentials out of declarations. Live fares, inventory, bookings, prices, orders, accounts and payments remain connectors to authoritative systems.',
    '',
    ...
    'A collection declaration does not create a submit tool, grant access, select retention or residency, provision storage, or expose records to MCP or an embedded assistant. Operators activate the definition and configure lifecycle, access, and any external binding. Native records retain legacy creation-time expiry until an Owner/Admin explicitly reviews and confirms preservation through Business settings or `noodle solutions records lifecycle`. Then expiry is null (until explicit erasure); activity history caps remain separate. Never change expiry in authored tools or infer an appointment was fulfilled from a saved request. A connector does not become a collection unless the app explicitly supplies the source contract, and an external collection never becomes a writable mirror.',
    '',
    '## Boundaries',
    '',
