...
export function renderEmbeddedAssistantReference(): string {
  return [
    '# Embedded assistant',
    '',
    '## Contents',
    ...
    '',
    '## WhatsApp public alpha',
    '',
    'Reuse the same server and public read-only capabilities with `publicMessaging({ channel: "whatsapp", capabilities: [product, siteIdentity] })` in `embeddedAssistant.access`. Import `publicMessaging` from `@noodleseed/one`. This projection is explicit: knowledge, pure public reads, and a `collect` opener listed with its confirmed native-record action. The platform’s natural collection gathers the opener’s fields in conversation, asks the consent question, renders the review verbatim and confirms; a field marked `private: true` never reaches the conversation model or the transcript. Other writes, connector-backed lookups, customer identity and browser widgets without an interaction are not supported. A WhatsApp participant is anonymous; never infer a customer account from its phone number.',
    '',
    'Operator setup uses `noodle channels whatsapp configure`, managed secret references, `webhook configure`, `doctor`, and `enable`. Start paused. Only a service advertising `features.whatsapp: 1`, durable storage, a running worker, a verified cost bound and matching provider webhook can enable. Use `noodle channels whatsapp --help` for the complete operator command catalog. Keep real keys in `noodle secrets`, never in source, flags or messages. ChatGPT and Claude continue to use the same app MCP endpoint with their own host configuration.',
    '',
    '## Collect interactions',
    '',
    'An opener `tool()` may declare `interaction: { kind: "collect", action, fields, review: "all", outcome }`: one definition of a form-like exchange that Noodle Seed presents with the controls each channel has. On the website the tool’s React `view` (or a standard form) collects the fields; on a messaging channel the platform renderer gathers the same fields in natural conversation, renders the review verbatim, and saves through the same confirmed `action`. The block carries meaning, never layout, ordering or screen language; `noodle validate` checks it against the action’s input schema.',
    '',
    '- `action` is a tool with `annotations.openAction({ confirm: true })` (never read-only) whose fulfilment saves one native record through `connectors.records`.',
    '- `fields` name the action’s Zod input keys with one of seven controls: `text`, `textarea`, `email`, `phone`, `url`, `select` (a `z.enum` of strings) and `consent` (`z.literal(true)`, at most one; the runtime poses the consent question and never infers it).',
    '- Nothing is private by default. `private: true` keeps that value out of the conversation model, transcript, logs and diagnostics. `optional: true` lets a person skip a field; an optional field the schema still requires must accept `""`.',
    '- `initialValues: { workflow: { fromOutput: "workflow" } }` seeds action inputs from the opener’s `output`; every other required action input must be a field.',
    '- `confirmationExpiry: { seconds }` may only shorten the channel’s default (ten minutes on WhatsApp), never lengthen it.',
    '- On `publicMessaging`, list the opener and its action together. The compiler reports `channel_dependency_missing` for an opener selected alone and `channel_requirement_unsupported` for a widget, write or external connector call that has no collect interaction.',
    '',
    '## Architecture',
    '',
    ...
    '',
    "Set `history: false` on a `publicWebsite`, `authenticatedWebsite` or `publicMessaging` surface whose chats must never be kept as conversation history, such as triage, health intake or an internal setup assistant. That surface then records nothing and states no retention window, whatever the business chose, while its other surfaces keep following the business setting, and `noodle solutions history settings get` names it as disabled by the application. Only `false` is accepted: whether and how many days a recorded surface keeps chats is the business's own setting (`noodle solutions history settings set`), never a number in `server.ts`, so omit the field everywhere else.",
    '',
    ...
    '',
    "When the business records conversation history, the response carries an optional top-level `history: { retentionDays, notice? }` for the current caller, and the widget states it in its footer automatically (\"Chats are kept for 30 days\"); WhatsApp states it in the first reply. To state it in your visitors' language, declare `embeddedAssistant({ historyNotice: 'Los chats se guardan {days} días', ... })` (`{days}` required); both channels then use it and the response carries the filled-in text as `history.notice`. A custom renderer must show the same line: read `history` from the `session_started` event data, render `history.notice` when present or the English line otherwise, and render nothing when `history` is absent. A surface declared `history: false` never carries it. State the same period on the host application's privacy page.",
    '',
    'Recorded conversations belong to the `user.id` the backend sent at exchange, so your backend can manage them with the same client credential. Call `forgetUser` from the application\'s own account-deletion flow (idempotent; retry on a retryable `AssistantConversationsError`), and `listConversations` for a "your chats" or support view (newest first, a short first-question `preview`, `nextCursor` paging). Pass the identical id used for `createAssistantSession`; any other id sees nothing. Both are backend-only and scoped to the client\'s application environment.',
    '',
    '```ts',
    'import { forgetUser, listConversations } from "@noodleseed/assistant/server";',
    '',
    'const credentials = { serviceUrl, clientId, clientSecret };',
    'const page = await listConversations({ ...credentials, user: { id: user.id }, limit: 20 });',
    'await forgetUser({ ...credentials, user: { id: user.id } }); // inside your account deletion',
    '```',
    '',
