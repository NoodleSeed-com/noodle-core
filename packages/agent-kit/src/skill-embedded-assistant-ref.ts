...
export function renderEmbeddedAssistantReference(): string {
  return [
    ...
    '',
    'When the business records conversation history, the response carries an optional top-level `history: { retentionDays }` for the current caller, and the widget states it in its footer automatically ("Chats are kept for 30 days"); WhatsApp states it in the first reply. A custom renderer must show the same line: read `history.retentionDays` from the `session_started` event data and render nothing when it is absent. A surface declared `history: false` never carries it. State the same period on the host application\'s privacy page.',
    '',
