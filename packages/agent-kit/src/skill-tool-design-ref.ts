// Curated tool-design reference: the doctrine a model-facing tool surface has to follow. Extracted
// verbatim from the authoring-workflow reference so it has its own route, then extended with the
// rules `noodle check` enforces. The public owner of this doctrine is the
// "Designing tools for agents" guide on docs.noodleseed.dev; keep the two in step.
//
// The `export default server(` worked example below is compile-verified on every `pnpm test` via
// scripts/verify-skill-snippets.mjs (the real `noodle validate`, not raw `tsc`).

export function renderToolDesignReference(): string {
  return [
    '# Tool design',
    '',
    '## Contents',
