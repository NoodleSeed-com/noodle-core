import type { AgentTarget, SkillDefinition } from './skill-registry.js';

export const EXECUTING_NOODLE_PLANS_SKILL = {
  name: 'executing-noodle-plans',
  description:
    'Use when the user asks to execute an approved, decision-complete implementation plan for a Noodle Seed project task by task with test-first changes, review, recovery, and final verification.',
  supportedHosts: ['codex', 'claude-code'],
  files: [],
  renderBody: renderExecutingNoodlePlansSkill,
} as const satisfies Omit<SkillDefinition, 'version'>;

export function renderExecutingNoodlePlansSkill(_target?: AgentTarget): string {
  return [
